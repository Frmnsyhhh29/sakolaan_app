...
class NilaiFormScreen extends ConsumerStatefulWidget {
  final int? nilaiId; // null = tambah, ada value = edit

  const NilaiFormScreen({super.key, this.nilaiId});

  @override
  ConsumerState<NilaiFormScreen> createState() => _NilaiFormScreenState();
}

class _NilaiFormScreenState extends ConsumerState<NilaiFormScreen> {
  ...
  Future<void> _loadNilaiData() async {
    ...
    try {
      ...
      final nilai = await nilaiService.getNilaiById(widget.nilaiId!);
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      bool success;
      if (widget.nilaiId == null) {
        success = await ref.read(nilaiProvider.notifier).addNilai(nilai);
      } else {
        success = await ref.read(nilaiProvider.notifier).updateNilai(widget.nilaiId!, nilai);
      }

      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.nilaiId == null
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.nilaiId == null ? 'Tambah Nilai' : 'Edit Nilai',
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          ...
                          child: _isLoading
                              ...
                              : Text(
                                  widget.nilaiId == null ? 'Simpan Nilai' : 'Update Nilai',
                                  style: const TextStyle(
                                    fontSize: 16,
                                    fontWeight: FontWeight.w600,
                                  ),
